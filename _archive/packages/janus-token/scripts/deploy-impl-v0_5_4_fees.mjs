/**
 * deploy-impl-v0_5_4_fees.mjs — Deploy JanusFlow_v0_5_4_fees implementation.
 *
 * Deploys ONLY the implementation — does NOT upgrade the proxy.
 * Run upgrade-proxy-to-v0_5_4_fees.mjs next.
 *
 * Run from package root:
 *   node scripts/deploy-impl-v0_5_4_fees.mjs
 */
...
async function main() {
    ...
    console.log("JanusFlow v0.5.4-fees — Deploy implementation contract");
    console.log("=".repeat(72));
    ...
    console.log("\n[1/1] Deploying JanusFlow_v0_5_4_fees impl...");
    const implBc = loadBytecode(ART_JANUSFLOW);
    console.log(`  bytecode: ${implBc.length / 2} bytes`);

    const implRes   = runFlowTx(DEPLOY_TX, [implBc], "v054fees_deploy_impl");
    ...
    if (!implAddr) {
        writeFileSync("/tmp/v054fees-impl-deploy-raw.json", JSON.stringify(implRes, null, 2));
        throw new Error("Failed to parse JanusFlow_v0_5_4_fees impl address — raw result in /tmp/v054fees-impl-deploy-raw.json");
    }
    ...
    const OUT = join(DEPLOYMENTS_DIR, "janusflow-fees.json");
    writeFileSync(OUT, JSON.stringify(record, null, 2) + "\n");
    console.log(`\nDeployment record: ${OUT}`);

    console.log("\n" + "=".repeat(72));
    console.log(`NEXT: node scripts/upgrade-proxy-to-v0_5_4_fees.mjs`);
    console.log("=".repeat(72));
    return record;
}
